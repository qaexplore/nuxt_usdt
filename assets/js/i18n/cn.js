...
// 交割合约
import regular from './cn/regular.js'
let ZH = {
  common: {
    success: '操作成功！',
    noData: '暂无数据',
  ...
  transform: {
    ...
    same: '转出账户和转入账户不能相同',
    "transferUsdtTip": "提示：在持仓状态下转出后，您的风险度将会重新计算。转出金额过多将会增大您的爆仓风险。为了避免不必要的损失请您在账户中预留足够的保证金以应对行情波动。有持仓状态下可以转出本金的{percentage}，浮盈可用但不可转出。无持仓状态下可以全部转出。",
    "transferRegularTip": "提示：在持仓状态下转出后，您的风险度将会重新计算。转出金额过多将会增大您的爆仓风险。为了避免不必要的损失请您在账户中预留足够的保证金以应对行情波动。有持仓状态下可以转出本金的{percentage}，浮盈可用但不可转出。无持仓状态下可以全部转出。已实现盈亏在结算计入本金后可以全部转出。"
  ...
  createdEntrust: {
    ...
    marketInfo: '市价：为防止误操作，减少客户损失。市价将按照指数价格±3%的价格进行委托，超过此范围价格将无法成交。',
    positiveNum: '请输入正数',
    ...
    explosion: '以该价格向市场委托可能导致强平，请先检查您是否有挂单，并以更合理的价格进行委托。',
    hand: '交易单位（手）：也称为合约规模，是指在交易所交易的每手合约代表的标的物的数量。<span style="display: block">eg: 1{hand}{name} = {value}{tableName}</span>',
  ...
  transactionList: {
    ...
    elpTips3: '多币种持仓需同时满足所有币种强平价格才能真正触发强平。系统不会对多持仓状态下的单独币种进行强平。',
    strongParity: '预估强平价',
    ...
    closeAll: '一键平仓',
    batchCancel: '批量撤单',
    prompt: '一键全平仅支持市价报单方式（针对可用持仓），可能会产生较大的风险，请谨慎使用。',
    currency: '币种',
    openLeve: '开平',
  ...
  tradeDetailList: {
    ...
    liquidationPriceTip: '如果该合约的标记价格低于该价格(多仓)或高于该价格(空仓),您将会被强制平仓',
    margin: '保证金',
    ratio: '回报率%',
  ...
  errorCode: {
    ...
    10300009: "资金互换中，暂停交易",
    10300011: "委托数量大于持仓数量，可用平仓数量不足",
    10300012: "部分委托未成交，可用平仓数量不足",
    10300013: "没有多仓位可平",
    10300014: "没有空仓位可平",
    10400000: "没有持仓",
