<template>
  <div
    :class="{'trade-history':'black' === themeValue, 'trade-history-white': 'white' === themeValue}"
  >
    <st-row class="main" direction="column">
      <st-row class="header" justify="between">
        <st-row class="left">
          <p
            :class="{'active': 'done' === showType}"
            @click="change('done')"
          >{{$t('message.orderDetail.done')}}</p>
          <p
            :class="{'active': 'cancel' === showType}"
            @click="change('cancel')"
          >{{$t('message.orderDetail.cancel')}}</p>
          <p
            :class="{'active': 'asset' === showType}"
            @click="change('asset')"
          >{{$t('message.orderDetail.asset')}}</p>
        </st-row>
        <div class="right">
          <div class="item">
            <input
            ...
            <z-select v-if="showContract" :list="allContractList" @selected="changePresentContract"></z-select>
          </div>
          <!--站点查询-->
          <!--<div class="item">-->
          <!--<input type="text"-->
...
export default {
  ...
  data() {
    return {
      ...
      siteList: [{ id: 1, name: "永续合约" }, { id: 2, name: "USDT合约" }],
      allList: null,
      selectIndex: 0
    };
  },
  created() {
    this.$store.state.market.contract.forEach((k, v) => {
      if (Number(v.id) === Number(this.$route.params.id)) {
        this.selectIndex = k;
      } else {
        this.selectIndex = 0;
      }
  ...
  watch: {
    ...
    "$i18n.locale": {
      handler() {
        this.presentContract = this.allContractList[this.selectIndex].name;
      }
    }
  },
  methods: {
    ...
    changePresentContract(val, index) {
