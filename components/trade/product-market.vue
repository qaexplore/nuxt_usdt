    ...
    <st-row class="top">
      ...
      <st-row class="market">
        <st-row class="item" direction="column">
          <p :class="{'value': true, 'green': Number(ticker.change) > 0, 'red': Number(ticker.change) < 0}">{{ticker.last || 0 | splitFormat(figure.indexPriceFloat)}} <span class="rate-money" v-if="rate !== 0 ">≈{{unit}}{{ (ticker.last * rate) | splitFormat(figure.indexPriceFloat) }}</span></p>
...
<script>
  ...
  export default {
    ...
    methods: {
      getRate () {
        AccountApi.getRateUsd(this.$i18n.locale === 'cn'? 0 : 1)
        .then(res => {
