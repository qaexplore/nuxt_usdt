<template>
  <div :class="{'trade-detail-position-new':'black' === themeValue, 'trade-detail-position-new-white': 'white' === themeValue}">
    <div class="table-wrapper" style="width: 100%;">
      <el-table :data="list" :max-height="tableHeight1" id="table1" class="el-table-content table1" v-if="list.length > 0"
        @row-click='choice'
        ...
        <el-table-column  prop="" min-width="100" align="right" :label="$t('message.transactionList.unrealisedPNL')" >
          <template slot-scope="scope">
            <span v-if="scope.row.id!=='last'" :class="{buyColor: getUnrealizedPL(scope.row)>0, sellColor: getUnrealizedPL(scope.row) <0 }" >{{ getUnrealizedPL(scope.row) | splitFormat(2) }}</span>
        ...
        <el-table-column  prop="" min-width="90" align="right" :label="$t('message.transactionList.profitability')" >
          <template slot-scope="scope">
            <span style="padding-right: 11px;" :class="{buyColor: getRate(scope.row)>0, sellColor: getRate(scope.row) <0 }" v-show="scope.row.id !== 'last'">{{ getRate(scope.row) + '%' }}</span>
          </template>
        </el-table-column>
        <!-- 预估强平价 -->
        <el-table-column  prop="" min-width="150" align="right" :label="$t('message.transactionList.strongParity')" :render-header="SHOW_TOP_TIPS">
          <template slot-scope="scope">
        ...
        <el-table-column fixed="right" align="center" prop="" :min-width="action_width"
          ...
          <template slot-scope="scope">
            <span :class="{'detail-position': themeValue === 'black', 'detail-position-white': themeValue === 'white'}">
              ...
              <a v-show="scope.row.id !== 'last'" @click="closeBtn(scope.row, 1)" id="closeTrade2" :class="{'button': true, 'button1': true, 'button3':true}" style="font-weight:600;">{{$t('message.transactionList.stopProfit')}}</a>
              <a v-show="scope.row.id !== 'last'" @click="closeBtn(scope.row, 2)" id="closeTrade3" :class="{'button': true, 'button1': true, 'button3':true}" style="font-weight:600;">{{$t('message.transactionList.stopLoss')}}</a>
    ...
    <popup :title="!showCloseConfirm ? $t('message.transactionList.A_Close') : ''"
           :callback="popClose"
           :boxWidth="'320px'"
    ...
    <!-- 止盈/止损弹框 -->
    <popup :title="profit_loss_title"
           :callback="popClose"
           :boxWidth="'320px'"
           :bottomB="bottomB"
           v-if="showLoss">
      <layer-loss :popData="popData" 
              :lossType = "lossType"
              :lossSide = "lossSide"
              @popClose="popClose"
              @confirm="lossConfirmCallback"></layer-loss>
    </popup>
    <!--一般提示框 toast 提示-->
  ...
  export default {
    ...
    data () {
      return {
        ...
        action_width: 140,
        // 止盈/止损（手）
        stopProfitHand: `${this.$t('message.transactionList.stopProfit')} (${this.$t('message.transactionList.A_Hand')})`,
        stopLossHand: `${this.$t('message.transactionList.stopLoss')} (${this.$t('message.transactionList.A_Hand')})`,
        timer: null
      }
    },
    computed: {
    ...
    watch: {
      local (val) {
        ...
        this.stopProfitHand = `${this.$t('message.transactionList.stopProfit')} (${this.$t('message.transactionList.A_Hand')})`;
        this.stopLossHand = `${this.$t('message.transactionList.stopLoss')} (${this.$t('message.transactionList.A_Hand')})`;
      },
      market (val) {
        // 计算未实现盈亏
        this.setUnrealiset();
    ...
    methods: {
      ...
      SHOW_TOP_TIPS (h, {column, $index}) {
        return (
          <font>
            ...
            <el-popover trigger="hover" popper-class="tooltip-content" placement="top">
              <p>
                <span>{this.$t('message.transactionList.elpTips1')}</span>
                <span style="color: #d64172;">{this.$t('message.transactionList.elpTips2')}</span>
                <span>{this.$t('message.transactionList.elpTips3')}</span>
              </p>
              <font slot="reference" class="tooltipinfo iconfont icon-wenhaofill"></font>
            </el-popover>
          </font>
        );
      },
      ...
      getUnrealizedPL (item) {
        ...
        if (item.side === 1) {
          ...
          return this.$filter.splitFormat(Utils.mul(Utils.mul(Utils.sub(this.market[item.contractId].indexPrice, item.entryPrice), item.positionQty), this.getInfo(item.contractId, this.contract).value), 2)
        } else {
          ...
          return this.$filter.splitFormat(Utils.mul(Utils.mul(Utils.sub(item.entryPrice, this.market[item.contractId].indexPrice), item.positionQty), this.getInfo(item.contractId, this.contract).value), 2)
        }
      },
      getRate (item) {
        if (!this.market[item.contractId] || !this.market[item.contractId].indexPrice) return 0
        if (item.side === 1) {
          return Utils.splitFormat(Utils.mul(Utils.div(Utils.sub(this.market[item.contractId].indexPrice, item.entryPrice), item.entryPrice), 100), this.getInfo(item.contractId, this.contract).contractParam.profitRateFloat)
        } else {
          return Utils.splitFormat(Utils.mul(Utils.div(Utils.sub(item.entryPrice, this.market[item.contractId].indexPrice), item.entryPrice), 100), this.getInfo(item.contractId, this.contract).contractParam.profitRateFloat)
        }
      },
      refresh () {
      ...
      setUnrealiset () {
        ...
        this.listData.forEach((v) => {
          unrealised = Utils.add(unrealised, this.$filter.splitFormat(this.getUnrealizedPL(v),2)*100);
        })
        this.$store.commit('SET_UNREALISED', (unrealised/100).toFixed(2));
      },
      popClose () {
        this.showPop = false
      ...
      SET_POP_TITLE (type, item) {
        ...
        this.contract.forEach((v) => {
          if (v.id == item.contractId) {
            symbol = v.name;
            bill_type = item.side == 1 ? this.$t('message.transactionList.moreBill') : this.$t('message.transactionList.emptyBill');
            this.lossSide = item.side;
          }
        });
        this.lossType = type;
        this.profit_loss_title = type == 1 ?  `${symbol}${bill_type} ${this.$t('message.transactionList.stopProfit')}` : `${symbol}${bill_type} ${this.$t('message.transactionList.stopLoss')}`;
      },
      // 止盈/止损操作
