<template>
  <div
    :class="{'trade-detail-current':'black' === themeValue, 'trade-detail-current-white': 'white' === themeValue}"
  >
    <div class="table-wrapper" style="width: 100%;">
      ...
      >
        ...
        >
          <template v-if="scope.row.side" slot-scope="scope">
            <span
              @click="showCancelAllLayer(scope.row.id,scope.row)"
    ...
    >
      <layer-cancel-all
        ...
        :showLayerCancelALlToastSuccess="showLayerSuccess"
      ></layer-cancel-all>
...
export default {
  ...
  data() {
    return {
      ...
      filtersList: [
        {
          text: this.$t("message.transactionList.all"),
          value: "all",
          click: false
        }
      ],
  ...
  watch: {
    showpop() {
      setTimeout(() => {
        document.getElementsByClassName("pop-filters")[0].style.display =
          "none";
      }, 10);
    },
  ...
  methods: {
                ...
                that.filtersList.map(function(item) {
                  return h(
                    ...
                    [
                      h("input", {
                        class:
                          that.checked.indexOf(item.value) >= 0
                        ...
                        attrs: {
                          ...
                          value: item.value,
                          checked:
                            that.checked.indexOf(item.value) >= 0 ? true : false
                        },
                        on: {
                          change(e) {
                      ...
                      h("span", {
                        ...
                        domProps: {
                          innerHTML:
                            item.value === "all"
                              ? that.$t("message.transactionList.all")
                              : item.text
                        }
                      })
                    ]
      ...
      SwapsApi.getOrderList({
        ...
        .then(res => {
          ...
          if (this.filterlists.length > 0) {
            this.$store.commit("market/SET_BATCH_WITHDRAWAL", true);
          } else {
            this.$store.commit("market/SET_BATCH_WITHDRAWAL", false);
          }
    ...
    revert() {
      ...
      SwapsApi.cancelOrder(this.contractId, this.cellId)
        .then(res => {
