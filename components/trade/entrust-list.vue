    ...
    <div class="list">
            ...
            <div class="td-center">
              <div class="info">
                <p class="new lastPrice-tip"
                   @click="setPrice(specialLastPrice)">
                  {{specialLastPrice | splitFormat(figure.indexPriceFloat) || 0}} <b class="rate-money" v-if="rate !== 0">≈{{unit}}{{ specialLastPrice * rate | splitFormat(figure.indexPriceFloat) }}</b>
      ...
      <st-row class="deep">
        ...
        <st-row class="deep-button">
          <a @click="mergeDeep(item)" :class="[deep === item ? 'active' : '']" v-for="(item, index) in deepList" v-if="index<2" :key="index">{{ item }}{{$t('message.entrustList.bit')}}</a>
        </st-row>
      </st-row>
    </div>
  </div>
...
//      }
    ...
    computed: {
      ...
      // K线最新成交价
      specialLastPrice () {
        return Number(this.$store.state.market.specialLastPrice) ? this.$store.state.market.specialLastPrice : this.lastPrice
      },
      sell () {
    ...
    watch: {
      ...
      buy (val) {
        ...
        if (this.sellOrder[this.showItem - 1].price && this.buyOrder[0].price && Number(this.buyOrder[0].price) >= Number(this.sellOrder[this.showItem - 1].price)) {
