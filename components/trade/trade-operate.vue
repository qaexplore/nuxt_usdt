  ...
  <div class="dis-selected" :class="{'trade-operate':'black' === themeValue, 'trade-operate-white': 'white' === themeValue}" @click="removeSelected">
    ...
    <st-row class="content" direction="column">
      ...
      <div class="box" style="margin-top: 5px;">
        <st-row class="input">
          <st-row class="item" :class="[orderType.value === 2 && 'input_disabled']">
            <span class="label" :class="[orderType.value !== 2 && 'focus', langCss]">{{ $t('message.createdEntrust.price') }}</span>
            <input type="text"
        ...
        <st-row class="input">
          <st-row class="item">
            <span class="label focus" :class="[langCss]">
              ...
              <div class="tooltip-div">
                <el-popover trigger="manual" v-model="popoverStatus2" popper-class="tooltip-content" :content="$t('message.createdEntrust.hand', {hand: $t(`message.createdEntrust.lot`), name: contractInfo.name, value: contractInfo.value, tableName: contractInfo.tableName})" placement="top">
                  <p v-html="$t('message.createdEntrust.hand', {hand: $t(`message.createdEntrust.lot`), name: contractInfo.name, value: contractInfo.value, tableName: contractInfo.tableName.toUpperCase()})"></p>
            ...
            <!-- 手数*amount -->
            <span class="amount_lot" v-if="Number(amountInfo) > 0 && (amountInfo+'').length <= 6">( = {{amountInfo}} {{contractInfo.tableName.toUpperCase()}})</span>
          </st-row>
      ...
      <st-row style="width: 100%;font-size: 12px;" direction="column" justify="flex-start">
        ...
        <div style="text-align: right">{{ $t('message.createdEntrust.positionValue') }}：{{positionValue | splitFormat(4)}} USDT</div>
      </st-row>
    </st-row>
    ...
    <st-row class="cover" justify="center" v-if="!userInfo">
      <div class="text">
        ...
        <p>
          ...
          <span>
            <a :href="apiConfig.mainDomain + 'manage/login?position=usdt'">{{ $t('message.noSign.sign_in')}}/</a>
  ...
  export default {
    ...
    watch: {
      ...
      amount (val) {
        ...
        this.amountInfo = Utils.mul(Number(this.contractInfo.value), Number(this.amount));
        this.calPositionValue(this.price,this.amount)
      },
      contractInfo: {
        handler (val) {
      ...
      replaceValid (val) {
        ...
        if (val === 'price') {
          ...
          if (this.price.indexOf('.') > -1) {
            ...
            this.price = this.price.replace(/\D/gi, '').slice(0, index) + '.' + this.price.replace(/\D/gi, '').slice(index, index + (this.figure.indexPriceFloat ? this.figure.indexPriceFloat : 4))
      ...
      calPositionValue (price, amount) {
        ...
        if (this.market[this.$route.params.id].indexPrice && amount) {
          ...
          if (this.orderType.value === 2) {
            this.positionValue = this.market[this.$route.params.id].indexPrice * amount * this.contractInfo.value
          } else {
