<template>
  <div :class="{'asset-information':'black' === themeValue, 'asset-information-white': 'white' === themeValue}">
    ...
    <div class="box" :class="[utils.splitFormat(utils.editE(utils.mul(walletAsset.risk || 0, 100)), 0) < 50 && 'safe_color']">
    ...
    <st-row class="content" direction="column">
      ...
      <st-row class="info" direction="column">
        ...
        <st-row class="item" justify="between">
          ...
          <p :class="{'value': true,'buyColor': utils.editE(!!unrealised ? unrealised : walletAsset.unRealised) > 0, 'sellColor': utils.editE(!!unrealised ? unrealised : walletAsset.unRealised) < 0 }" v-if="userInfo">{{ utils.editE(!!unrealised ? unrealised : walletAsset.unRealised) | splitFormat(figure.fundFloat) || 0 }}</p>
          <p class="value" v-else>-</p>
        </st-row>
        ...
        <st-row class="item" justify="between">
          ...
          <p class="value" v-if="userInfo">{{ (fundUtilizationRate * 100) | splitFormat(2) || 0 }}<font v-if="fundUtilizationRate">%</font></p>
          <p class="value" v-else>-</p>
        </st-row>
  ...
  export default {
    ...
    methods: {
      ...
      fundleverageUtilizationRate (obj) {
        ...
        this.fundUtilizationRate = Number(obj.current) !== 0 ? (Number(obj.positionFrozen) + Number(obj.orderFrozen)) / Number(obj.current) : 0;
        ...
        let list_obj_arr = {};
        let sub_number = 0;
        lists.length > 0 && lists.forEach( (v) => {
        ...
        for(let id in list_obj_arr){
          ...
          let number;
          if (arr.length == 1) {
            number = Number(arr[0].entryPrice) * Number(arr[0].positionQty) * Number(arr[0].contractValue);
          } else {
            let index = Number(arr[0].positionQty) >= Number(arr[1].positionQty) ? 0 : 1;
            number = (Number(arr[index].positionQty) - Number(arr[index == 0 ? 1 : 0].positionQty)) * Number(arr[index].entryPrice) * Number(arr[index].contractValue);
          }
          sub_number += number;
        }
        this.fundLeverage = Number(obj.current) !== 0 ? sub_number / Number(obj.current) : 0;
      },
      showTransform () {
        if (this.userInfo) {
      ...
      getWallet () {
        ...
        this.walletTimers++;
        if ((!this.userInfo || !Cookie.getCookie('token')) && this.walletTimers >= 3) {
