  ...
  <div class="fund">
    ...
    <ul>
      <li
        ...
        :key="index"
      >
        <router-link :to="'/handbook/insurance/' + item.id">{{ item.name | contractCoin }}</router-link>
      </li>
    </ul>
    ...
    <st-row class="toHistory" justify="center" align="center">
      ...
      <router-link
        ...
        v-html="$t('message.direct.navigation.secondaryTitle5')"
      ></router-link>
    </st-row>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getRatioList() {
      ...
      let list = [];
      newSwapApi.getInsuranceFundList(this.orderData).then(res => {
