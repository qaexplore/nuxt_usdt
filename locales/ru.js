...
// 交割合约
import regular from './ru/regular.js'
let ZH = {
  common: {
    success: 'Готово',
    noData: 'Нет данных',
  ...
  header: {
    ...
    regularContract: 'Фьючерсный контракт',
    guide: 'Руководство',
  ...
  productInfo: {
   ...
   createdEntrust: {
    ...
    positions: 'позиции',
    lot: 'лот',
     leverageInfo: "Кредитное плечо: в настоящее время предлагает многократное кредитное плечо.Высокое кредитное плечо увеличит риск, пожалуйста, используйте с осторожностью.\n",
    ...
    A_limit: 'Предел. цена',
    A_buy_title: 'Покупка<span style="color:{color};">{num}</span>{name}контрактов',
    A_sell_title: 'Продажа<span style="color:{color};">{num}</span>{name}контрактов',
    A_buy_flat: 'Закрытие позиций на покупку',
    A_sell_flat: 'Закрытие позиций на продажу',
     marketInfo: 'Рыночная цена: для того чтобы предотвратить неправильное использование и уменьшить потери клиента. Рыночная цена будет введена в соответствии с индексной ценой±3%, от цены ордера, превышающие этот диапазон, не будут торговать.',
    ...
    hand: 'Единица торгов (лот): также известный как мера контракта, это количество лотов, представленных в каждой сделке, торгуемой на бирже.<span style="display: block">eg: 1{hand}{name} = {value}{tableName}</span>',
     explosion: 'Делегирование на рынок по этой цене может привести к ликвидации позиций, пожалуйста, проверьте, есть ли у вас отложенные ордера и делегируйте их по более разумной цене.',
  ...
  asset: {
    ...
    riskDegree: 'Риск',
    riskDegreeInfo: 'Когда уровень риска достигает 100%, запускается процесс ликвидации. Пожалуйста, обращайте внимание на уровень риска.',
    transform: 'Перевод',
    transfer: 'Перевод'
  },
  // 列表
  transactionList: {
    ...
    elpTips2: 'Ориентировочная цена ликвидации не является фактической ценой ликвидации, и расчитывается только для справок.',
    elpTips3: 'Мультивалютные позиции должны одновременно достичь цены ликвидации, чтобы действительно запустить ликвидацию. Система не ликвидирует отдельные позиции в состоянии мульти-позиции.',
    ...
    newOrder: 'Не запущенные',
    SuccTrigger: 'Запущенные',
    canceled: 'Отмененные',
    triggerFailed: 'Ошибка запуска',
    ...
    prompt: 'Это действие поддерживает только рыночные котировки (для доступных позиций), что может привести к большим рискам. Пожалуйста, используйте с осторожностью!',
    currency: 'Валюта',
    openLeve: 'Ликвидация',
